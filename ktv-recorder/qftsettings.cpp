...
void QFTSettings::saveFTSettings()
{
   ...
   pDb->setPassword("AllowAdultEnc", QString::number((int)ui->checkAdult->checkState() + ALAD_STAT_SHIF));
...
{
   ui->checkAdult->setChecked(false);
   ui->lineUsr->setText("");
   ui->linePass->setText("");
...
{
   if ( ui->lineUsr->text() == "" || ui->linePass->text() == "" || ui->lineErosPass->text() == "" )
   {
        QMessageBox::critical(this, tr("Note!"),
            tr("<b>Please enter correct Values for Abonemet, Password and Parent Password.</b>\n</ul>\n"));
