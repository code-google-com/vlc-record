...
\----------------------------------------------------------------- */
int CShowInfo::autoUpdate(uint uiTime, QVector<cparser::SEpg> &vEpgList)
{
   int iRV = -1;
   uint uiS, uiE;

   if ( eShowType == ShowInfo::Archive ) // Archive Show
   {
   ...
   else if ( eShowType == ShowInfo::Live ) // Live Show
   {
       for (int i = 0; i < vEpgList.size(); i ++)
       {
           uiS = vEpgList[i].uiGmt;
           uiE = ((i + 1) < vEpgList.size()) ? vEpgList[i + 1].uiGmt : 0;

           if ( CSmallHelpers::inBetween(uiS, uiE, uiTime) )
           {
               sShowName = vEpgList[i].sName;
               uiStart = uiS;
               uiEnd = uiE;
               uiJumpTime = (uiTime > uiStart) ? uiTime : 0; // take care of relative time jumps!
               sDescr = QString(TMPL_BACKCOLOR)
                               .arg("rgb(255, 254, 212)")
                               .arg(createTooltip(sChanName,
                                QString("%1 %2").arg(sShowName).arg(vEpgList[i].sDescr),
                                uiStart, uiEnd));
               iRV = 0;
               break;
           }
